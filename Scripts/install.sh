...
if [ ! -d "$HOME/UnityDownloads" ]; then
	...
	echo 'UnityDownloads directory found.'
	ls -a $HOME/UnityDownloads
fi
...
if [ ! -f "$HOME/UnityDownloads/Unity_webgl.pkg" ]; then
	echo 'Downloading Unity 5.5.0 WebGL Build Support pkg:'
	curl --retry 5 -o $HOME/UnityDownloads/Unity_webgl.pkg http://download.unity3d.com/download_unity/38b4efef76f0/MacEditorTargetInstaller/UnitySetup-WebGL-Support-for-Editor-5.5.0f3.pkg
	if [ $? -ne 0 ]; then { echo "Download failed"; exit $?; } fi
else
...
sudo installer -dumplog -package $HOME/UnityDownloads/Unity.pkg -target /
echo 'Installing Unity_webgl.pkg'
sudo installer -dumplog -package $HOME/UnityDownloads/Unity_webgl.pkg -target /
